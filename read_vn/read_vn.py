...
#    VN_DIR = '/data/capri_test_data/VN/wget/GPM/2ADPR/HS/V06A/1_21'
    ...
    for root, dirs, files in os.walk(VN_DIR, topdown=False):
        for file in files:
            ...
            if len(site_pattern)>0:
                if file.split('.')[1].startswith(site_pattern):
                    do_file=True
                else:
                    do_file = False
            else:
            ...
            if file.endswith('.nc.gz') and do_file:
                if process_parquet_meta:
                    parquet_output_file = os.path.join(OUT_DIR,file+'.parquet')
                    json_output_file = os.path.join(OUT_DIR,file+'.json.gz')
                    ...
                    if upload_parquet:
                        ...
                        parquet_key = s3_dir+'/'+file+'.parquet'
                        upload_s3(os.path.join(OUT_DIR,file+'.parquet',), s3_bucket, parquet_key,overwrite_upload_flag)

                    if upload_meta:
                        #print("uploading metadata "+os.path.join(META_DIR,file+'.meta.json'))
                        metadata_key = meta_dir+'/'+file+'.meta.json'
                        upload_s3(os.path.join(META_DIR,file+'.meta.json'), s3_bucket, metadata_key,overwrite_upload_flag)
                ...
                if upload_bin:
                    ...
                    if os.path.isfile(os.path.join(root, file + '.gpm.bin')):
                        upload_s3(os.path.join(root,file+'.gpm.bin'), s3_bucket, 'bin/'+file+'.gpm.bin',overwrite_upload_flag)
                    if os.path.isfile(os.path.join(root, file + '.mrms.bin')):
                        upload_s3(os.path.join(root,file+'.mrms.bin'), s3_bucket, 'bin/'+file+'.mrms.bin',overwrite_upload_flag)

                if upload_img:
                    # check for GPM and MRMS DL images and kml files
                    # if os.path.isfile(os.path.join(root, file + '.gpm.bw.png')):
                    #     upload_s3(os.path.join(root,file+'.gpm.bw.png'), s3_bucket, 'img/'+file+'.gpm.bw.png',overwrite_upload_flag)
                    # if os.path.isfile(os.path.join(root, file + '.gpm.bw.kml')):
                    #     upload_s3(os.path.join(root,file+'.gpm.bw.kml'), s3_bucket, 'img/'+file+'.gpm.bw.kml',overwrite_upload_flag)
                    if os.path.isfile(os.path.join(root, file + '.gpm.col.png')):
                        upload_s3(os.path.join(root,file+'.gpm.col.png'), s3_bucket, 'img/'+file+'.gpm.col.png',overwrite_upload_flag)
                    if os.path.isfile(os.path.join(root, file + '.gpm.col.kml')):
                        upload_s3(os.path.join(root,file+'.gpm.col.kml'), s3_bucket, 'img/'+file+'.gpm.col.kml',overwrite_upload_flag)

                    # if os.path.isfile(os.path.join(root, file + '.mrms.bw.png')):
                    #     upload_s3(os.path.join(root,file+'.mrms.bw.png'), s3_bucket, 'img/'+file+'.mrms.bw.png',overwrite_upload_flag)
                    # if os.path.isfile(os.path.join(root, file + '.mrms.bw.kml')):
                    #     upload_s3(os.path.join(root,file+'.mrms.bw.kml'), s3_bucket, 'img/'+file+'.mrms.bw.kml',overwrite_upload_flag)
                    if os.path.isfile(os.path.join(root, file + '.mrms.col.png')):
                        upload_s3(os.path.join(root,file+'.mrms.col.png'), s3_bucket, 'img/'+file+'.mrms.col.png',overwrite_upload_flag)
                    if os.path.isfile(os.path.join(root, file + '.mrms.col.kml')):
                        upload_s3(os.path.join(root,file+'.mrms.col.kml'), s3_bucket, 'img/'+file+'.mrms.col.kml',overwrite_upload_flag)
